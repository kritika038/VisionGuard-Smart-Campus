...
def _student_payload(data: dict):
    ...
    first_name = data.get("first_name") or (full_name.split(" ", 1)[0] if full_name else "")
    last_name = data.get("last_name") or (full_name.split(" ", 1)[1] if " " in full_name else "")

    enrollment_no = data.get("enrollment_no") or data.get("roll_number") or data.get("roll_no")
    ...
    return {
        ...
        "department": str(data.get("department") or data.get("class") or "").strip(),
        "course": str(data.get("course") or "").strip(),
        "semester": str(data.get("semester") or "").strip(),
        "section": str(data.get("section") or "").strip(),
        "scholar_type": str(data.get("scholar_type") or "").strip(),
...
def get_students(db: Session = Depends(get_db)):
    try:
        result = db.execute(text("SELECT * FROM students ORDER BY id DESC"))
...
def save_face(data: dict, db: Session = Depends(get_db)):
    ...
    try:
        db.execute(
            text("UPDATE students SET photo_path=:photo_path WHERE id=:student_id"),
            {
...
def delete_student(student_id: int, db: Session = Depends(get_db)):
    try:
        db.execute(text("DELETE FROM students WHERE id=:student_id"), {"student_id": student_id})
        db.commit()
        return {
            "success": True,
